...
export class App {
  ...
  grammarSuggestionsList: { message: string; replacement: string }[] = [];
  correctedText = '';
  ...
  async exportReport() {
    try {
      ...
      const files = snap.docs
        .map((doc: any) => doc.data() as { filePath?: string; diskId?: string; metadata?: any })
        .filter((data: any) => data.metadata && Object.keys(data.metadata).length > 0);
      if (files.length === 0) {
      ...
      const allKeys = Array.from(
        new Set(
          files.reduce(
            (acc: string[], f: { metadata?: any }) => acc.concat(Object.keys(f.metadata || {})),
            []
          )
        )
      ) as string[];
      ...
      const rows = files.map((f: any) => {
        const row = [
          ...
          ...allKeys.map(
            (k: string) =>
              '"' +
              (f.metadata && typeof f.metadata[k] !== 'undefined'
                ? String((f.metadata as any)[k])
                : '') +
              '"'
          ),
        ];
  ...
  async checkGrammar() {
    const text = this.metadataEpisodeDetails?.trim();
    if (!text) {
      this.grammarMessage = 'No text to check.';
      this.grammarSuggestionsList = [];
      this.grammarChecked = false;
      setTimeout(() => (this.grammarMessage = ''), 4000);
      return;
    }

    this.checkingGrammar = true;
    this.grammarChecked = false;
    this.grammarMessage = 'Checking...';
    ...
    try {
      ...
      if (matches.length === 0) {
        this.correctedText = this.toSentenceCase(this.correctedText);
        this.grammarMessage = '✅ No issues found (Sentence cased).';
        this.checkingGrammar = false;
        this.grammarChecked = true;
        return;
      }

      // Sort matches in reverse order to safely replace text by offset
      const sorted = [...matches].sort((a, b) => b.offset - a.offset);
      for (const m of sorted) {
        if (m.replacements?.length) {
          const replacement = m.replacements[0].value;
          this.grammarSuggestionsList.push({
            message: m.message,
            replacement
          });
          this.correctedText =
            this.correctedText.slice(0, m.offset) +
            replacement +
            this.correctedText.slice(m.offset + m.length);
        }
      }

      // Apply sentence casing after corrections
      this.correctedText = this.toSentenceCase(this.correctedText);
      ...
      console.error(err);
      this.grammarMessage = '⚠️ Error checking grammar.';
    } finally {
      this.checkingGrammar = false;
    }
  }
  ...
  applyCorrections() {
    ...
    this.metadataEpisodeDetails = this.correctedText;
    this.grammarSuggestionsList = [];
    this.grammarMessage = '✅ Corrections applied with Sentence Case.';
    this.grammarChecked = false;
    setTimeout(() => (this.grammarMessage = ''), 4000);
  }

  /**
   * Converts a paragraph of text into sentence case.
   * Each sentence starts with an uppercase letter.
   */
  private toSentenceCase(text: string): string {
    if (!text) return '';
    // Normalize spacing and lowercase everything first
    text = text.trim().replace(/\s+/g, ' ');
    // Split sentences using ., ?, ! boundaries
    ...
    return sentences.join('').replace(/\s+([,.!?])/g, '$1'); // remove extra space before punctuation
  }
  ...
  playFile(filePath: string) {
    ...
    if (
      filePath &&
      (filePath.startsWith('file:///') || /^[a-zA-Z]:\\/.test(filePath) || filePath.startsWith('/'))
    ) {
  ...
  copyFile(filePath: string) {
    ...
    const normalizedPath = this.firebase.normalizePath(filePath);
    const fileObj = this.selectedFiles.find((f: any) => {
      const relPath = (f as any).webkitRelativePath || f.name;
      return this.firebase.normalizePath(relPath) === normalizedPath;
    });
    if (fileObj) {
  ...
  isFileAccessible(filePath: string): boolean {
    const normalizedPath = this.firebase.normalizePath(filePath);
    return this.selectedFiles.some((f: any) => {
      const relPath = (f as any).webkitRelativePath || f.name;
      return this.firebase.normalizePath(relPath) === normalizedPath;
    });
  }
  ...
  updateFilePath(filePath: string) {
    ...
    input.onchange = async () => {
      ...
      try {
        const col = this.firebase.getCollection('files');
        const snap = await this.firebase.getDocs(
          this.firebase.query(col, this.firebase.where('filePath', '==', filePath))
        );
        if (snap.empty) {
        ...
        const newPath = this.firebase.normalizePath(
          (matchingFile as any).webkitRelativePath || matchingFile.name
        );
  ...
  async loadDiskIdsFromFiles() {
    try {
      ...
      const diskIds = Array.from(
        new Set(files.map((f: any) => f.diskId).filter((id: any) => !!id))
      ) as string[];
  ...
  ngOnInit(): void {
    ...
    window.addEventListener('beforeunload', (event) => {
      ...
      event.returnValue =
        'Would you like to back up your data before closing? Go to the Export tab and click "Backup Data" to save as JSON.';
    });
  }
  ...
  async initFirebase() {
    try {
      ...
      alert(
        'Failed to initialize Firebase. Please check your Firebase configuration and reload the page.'
      );
    }
  }
  // Search tab state
  ...
  async searchFiles() {
    try {
      const col = this.firebase.getCollection('files');
      let constraints = [];
      if (this.searchDiskId)
        constraints.push(this.firebase.where('diskId', '==', this.searchDiskId));
      if (this.searchProgramme)
        constraints.push(
          this.firebase.where('metadata.Name of Programme', '==', this.searchProgramme)
        );
      if (this.searchEpisodeNumber)
        constraints.push(
          this.firebase.where('metadata.Episode Number', '==', this.searchEpisodeNumber)
        );
      if (this.searchTelecastDate)
        constraints.push(
          this.firebase.where('metadata.Telecast Date', '==', this.searchTelecastDate)
        );
      ...
      let results = snap.docs.map((doc: any) => {
        ...
        return {
          ...
          metadata: Object.entries(data.metadata || {})
            .map(([k, v]) => `${k}: ${v}`)
            .join('; '),
        };
      });
      ...
      results = results.filter((file: any) => {
        const diskMatch =
          !this.searchDiskId || file.diskId.toLowerCase() === this.searchDiskId.toLowerCase();
        const programmeMatch =
          !this.searchProgramme ||
          file.programme.toLowerCase() === this.searchProgramme.toLowerCase();
        ...
        const metadataMatch =
          !this.searchMetadata ||
          file.metadata.toLowerCase().includes(this.searchMetadata.toLowerCase());
  ...
  async editMetadata(filePath: string) {
    ...
    const normalizedPath = this.firebase.normalizePath(filePath);
    let fileObj = this.selectedFiles.find((f: any) => {
      const relPath = (f as any).webkitRelativePath || f.name;
      return this.firebase.normalizePath(relPath) === normalizedPath;
    });
    if (!fileObj) {
  ...
  async loadMetadataFields(filePath: string) {
    try {
      ...
      const file = (snap.docs[0].data() as any) || {};
      ...
      this.customFields = Object.entries(file.metadata || {})
        .filter(
          ([k]) =>
            !['Name of Programme', 'Episode Number', 'Episode Details', 'Telecast Date'].includes(k)
        )
  ...
  async saveMetadata() {
    ...
    try {
      const col = this.firebase.getCollection('files');
      const snap = await this.firebase.getDocs(
        this.firebase.query(col, this.firebase.where('filePath', '==', this.metadataFilePath))
      );
      if (snap.empty) {
        alert('File not found!');
        return;
      ...
      if (this.metadataProgramme) {
        const exists = this.programmes.some(
          (p) => p.name.toLowerCase() === this.metadataProgramme.trim().toLowerCase()
        );
        if (!exists) {
  ...
  async scanDisk() {
    ...
    const filteredFiles = this.selectedFiles.filter((f: any) => {
      const relPath = (f as any).webkitRelativePath || f.name;
    ...
    try {
      ...
      this.scanFiles = snap.docs
        .map((doc: any) => doc.data() as { filePath?: string; metadata?: any })
        .filter((data: any) => !data.metadata || Object.keys(data.metadata).length === 0)
        .map((data: any) => data.filePath || '');
    } catch (e) {
      this.scanProgressActive = false;
  ...
  async loadScannedFiles() {
    try {
      ...
      if (this.selectedScannedDiskId) {
        files = files.filter((data: any) => data.diskId === this.selectedScannedDiskId);
      }
      this.scannedFiles = files.map((data: any) => ({
        filePath: data.filePath || '',
        diskId: data.diskId || '',
      }));
    } catch (e) {
  ...
  async loadDisks() {
    try {
      ...
      const snap = await this.firebase.getDocs(col);
      this.disks = snap.docs.map((doc: any) => {
  ...
  async loadProgrammes() {
    try {
      ...
      const snap = await this.firebase.getDocs(col);
      this.programmes = snap.docs.map((doc: any) => {
  ...
  async deleteProgramme(prog: { id: string; name: string }) {
    if (confirm('Are you sure you want to delete this programme?')) {
      if (!confirm('Are you sure you want to delete this programme? This cannot be undone.'))
        return;
      try {
        const ref = this.firebase.getDoc('programmes', prog.id);
        await this.firebase.deleteDoc(ref);
  ...
  async checkPassword() {
    ...
    try {
      ...
      this.firebase.onAuthStateChanged(async (user: any) => {
        this.loading = false;
  ...
  async backupData() {
    try {
      ...
      const backupData = {
        files: filesSnap.docs.map((doc: any) => doc.data()),
        programmes: progsSnap.docs.map((doc: any) => doc.data()),
        disks: disksSnap.docs.map((doc: any) => doc.data()),
      };
